# ラテカピュータ 設計状態ファイル

> このファイルはAIエージェントの引き継ぎ用です。作業開始時に必ず読み込んでください。
> 更新日: 2026-04-21

---

## プロジェクト概要

ラテカピュータ：ラテカセ（カセットテープケース）にコンピュータを組み込む自作PC/マイコンプロジェクト。

- コンピュータ部メインボード: ESP32-C6 (Waveshare C6)
- 表示: 電子ペーパー or 液晶（フロントパネル）
- 電源管理: PCF8574 I2C (0x20) によるGPIO制御
- キーボード: ABCDEFG配列（ラテカピュータキーボード、CH32V003 + ESP32-C6、BT兼用）
- TinyBASICエミュレータ内蔵予定

---

## 筐体構成パーツ（コンピュータ部）

| # | パーツ名 | 状態 |
|---|---------|------|
| 1 | ラテカセ底部ジョイントアダプタ | 設計中（v2試作済み、v3形状確定待ち） |
| 2 | U字パーツ | 暫定v1スクリプト作成済み（寸法未確定） |
| 3 | フロントパネル | 未着手 |
| 4 | 背面パネル | 未着手 |
| 5 | 左パネル | 未着手 |
| 6 | 右パネル | 未着手 |
| 7 | 底部パネル | 未着手 |

---

## 現在の作業状況

### 完了済み
- ジョイントアダプタ v3 寸法確定・再設計（実測スケッチ入手後） → 完了（joint_adaptor_v3.py）
- ジョイントアダプタ v1, v2 のCadQuery Pythonスクリプト作成・STEP出力
- PCF8574電源管理UIのESP32ファームウェア実装
- i2c_scanツール作成
- MCP開発環境構築（CadQuery MCP・KiCad MCP・.mcp.json・.claudeignore）

### 進行中
（なし）

### 次にやること
- U字パーツ 設計（ハンドル穴位置・はめ込み構造の実測スケッチ入手後）
- フロントパネル設計（液晶パネル配置）
- 背面パネル設計（スイッチ電源排熱露出考慮）
- 左パネル・右パネル設計
- 底部パネル設計

---

## 設計上の重要な制約・寸法

> ※実測値が判明次第ここに追記すること

- ラテカセ底部：前部・後部・左右部で断面形状が異なる
- 左右部：U字パーツとの重なり部分で別断面になる
- フロント：液晶パネル配置のため一部断面形状が異なる
- 背面：スイッチ電源の排熱露出のため形状考慮が必要
- ジョイントアダプタの固定方法：
  1. ラテカセ底部の台錐穴 + ねじ止め（M3）
  2. ハンドル穴を活用したU字パーツとの連結

### v2の主要寸法（確定値）
- 外形: 440 x 263 mm、板厚 2mm
- 外周壁: H5 x T2 mm
- 中央抜き: 416 x 216 mm（十字形、4隅30mm角残し）
- 前端実体: 40mm / 後端実体: 7mm
- コーナー嵩上げ: 2mm
- 台錐: R7.0→R6.8 / H5mm / M3穴φ3.2mm
- コーナー中心位置: X=±193.0mm、Y=+76.5mm(前)/-109.5mm(後)

---

## ファイル構成

```
ratecaputa/honai/               ← このフォルダ（起動レベル）
├── design_state.md             # このファイル（★常に最新に保つ）
├── waveshare-c6/               # Waveshare ESP32-C6 ファームウェア（ESP-IDF）
├── xiao-c6/                    # XIAO ESP32-C6 ファームウェア（PlatformIO）
├── raspberry-pi/               # Raspberry Pi 4（未着手）
├── windows/                    # Windows連携（未着手）
├── cad/                        # 本体筐体 CAD（CadQuery）
│   ├── joint_adapter_v1.py
│   ├── joint_adapter_v2.py
│   ├── joint_adapter_v3.py
│   └── ラテカピュータのコンピュータ部筐構成について.txt
└── i2c_scan/                   # I2Cスキャンツール
```

---

## 環境・ツールチェーン

| 用途 | ツール | 状態 |
|------|--------|------|
| 3D CAD | CadQuery 2.7.0 | ~/mcp-env で稼働中 |
| CadQuery MCP | latecaputa-cadquery (5 tools) | ✓ 稼働中 |
| KiCad MCP | latecaputa-kicad (6 tools) | ✓ 構築済み（KiCadインストール待ち） |
| ファームウェア | ESP-IDF (idf.py) | 設定済み |
| PCB設計 | KiCad | インストール要 |

---

## AIエージェントへの引き継ぎ指示

1. このファイルと `cad/joint_adapter_v2.py` を最初に読むこと
2. CadQueryスクリプト実行: `~/mcp-env/bin/python3 cad/<script>.py`
3. MCPツール `cadquery_run` でスクリプト実行可能
4. 作業完了後は「現在の作業状況」と「次にやること」を更新すること
